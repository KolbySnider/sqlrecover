...
///
/// Kept as a separate pass from the SQLite carver rather than unified
/// with it -- the two have very different EOF strategies (SQLite's is a
/// single page_count * page_size formula; these formats each need their
/// own footer search or chunk walk), and keeping them apart means this
/// code can't accidentally regress the SQLite carving path's determinism.
...
/// GIF, PDF), each with either a header size field or a well-defined
/// terminator, which keeps false-positive risk low; plain-text carving
/// (nothing to key off) isn't attempted.
/// @param image_path Path to the raw partition image.
/// @param out_dir Output directory for recovered files (created if missing).
/// @param verbose If true, log each carve to stderr.
/// @param workers Worker threads for the scan and extract phases. At least 1.
