...
/// a schema attached, just a row of typed values. This module keeps a
/// small catalog of well-known Android schemas (SMS, call log, contacts,
/// ...) and tries to match rows against them by type fingerprint, so a
/// row can get labelled "android_sms" with named columns instead of
/// staying an anonymous value array.
///
...
/// anything; IntLike matches Int plus the 0/1 const encodings; TextLike
/// matches Text; Numeric matches Int or Real.
enum class ColClass { Any, IntLike, TextLike, Numeric };

/// @brief How to turn a raw column value into something readable, e.g.
/// "date: 1700000043000" -> "2023-11-14T22:14:03Z", "type: 2" -> "sent".
enum class Interp {
    None,
...
/// @brief One column in a known artifact: name, type constraint, optional
/// decoder.
...
/// @return false (leaving out untouched) if there's nothing to do:
/// Interp::None, a NULL value, wrong storage type, an out-of-range
/// timestamp, etc.
...
/// @return has_timeline is false for artifacts with no real event
/// timestamp (contacts, for instance) or names the catalog doesn't
/// recognize.
