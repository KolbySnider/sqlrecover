...
/// @param visited_live Currently unused; reserved for a future "skip
/// pages still in use" check.
...
/// get their cells decoded directly, the same way sqlite3's own
...
/// @param db Source database.
/// @param visited Pages already touched by the live walk. Must already be
/// fully populated by walk_table_btree (sized to page_count()+2) before
/// this is called -- it's read, not written, here, so it's safe to share
/// across threads as long as nothing is still filling it in.
